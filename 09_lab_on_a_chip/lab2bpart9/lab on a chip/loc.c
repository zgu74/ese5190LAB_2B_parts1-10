...
const uint CAPTURE_N_SAMPLES = 700;
...
void core1_main() {
    ...
    while(true){
        ...
        while (gpio_get(TRIGGER_PIN) == 0) 
        {
            sleep_ms(1000);
            printf("r:%d, g:%d, b:%d, c:%d\n", r, g, b, c);
            do{} while (gpio_get(TRIGGER_PIN) == 0);
        }
        /*
...
int main() {
    ...
    while(true){   
        ...
        {
            case 0:
                neopixel_set_rgb(0xff0000);
                break;
            case 1:
                neopixel_set_rgb(0x00ff00);
                break;
            case 2:
                neopixel_set_rgb(0x0000ff);
                break;
        }



        logic_analyser_arm(pio, sm, dma_chan, capture_buf, buf_size_words, 12, false); //12

        printf("Start recording WS2812\n");
        
        dma_channel_wait_for_finish_blocking(dma_chan);
        printf("Recording done!\n");

        print_capture_buf(capture_buf, 12, 1, CAPTURE_N_SAMPLES);

        logic_analyser_arm(pio, sm, dma_chan, capture_buf, buf_size_words, 22, false);//22
        logic_analyser_arm(pio, sm, dma_chan, capture_buf, buf_size_words, 23, false);//23

        printf("Start recording APDS9960\n");
        
        dma_channel_wait_for_finish_blocking(dma_chan);
        printf("Recording done!\n");

        print_capture_buf(capture_buf, 22, 2, CAPTURE_N_SAMPLES);
