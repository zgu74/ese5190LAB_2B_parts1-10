...
void a0_output (const uint8_t *buf, uint n_sample, double velocity, uint times) {
    printf("Start A0 output...\n");
    uint32_t t = (uint32_t)(10000 / velocity);
    for (int i = 0; i < times; ++i) {
        for (int index = 0; index < n_sample; ++index) {
            if (judge_value(buf, index)){
...
int main() {
    ...
    printf("    b           record boot\n");
    printf("    k           record key press\n");
    printf("    p           play record\n");
    printf("    0           A0 output\n");
    printf("\n");
    while(true) {
        ...
        switch(key){
            ...
            case 'o':
                ...
                printf("\nThe record will be play at %f of default speed and %d times\n", velocity, times);
                a0_output(capture_buf, CAPTURE_N_SAMPLES, velocity, times);
                printf("Done!\n");
                break;
